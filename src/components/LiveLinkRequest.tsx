...
export const LiveLinkRequest: React.FC<LiveLinkRequestProps> = ({ study, onSubmit }) => {
                ...
                <div className="bg-white rounded-md border border-green-200 p-3">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-mono text-gray-900 break-all">{study.redirectLink}</p>
                    </div>
                    <button
                      onClick={() => study.redirectLink && copyToClipboard(study.redirectLink)}
                      className="ml-3 p-2 text-green-600 hover:text-green-700 hover:bg-green-100 rounded-md"
                      title="Copy redirect link"
                      disabled={!study.redirectLink}
                    >
                      <Copy size={16} />
                    </button>
                  </div>
