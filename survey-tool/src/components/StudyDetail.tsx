    ...
    <div className="space-y-8 fade-in">
      ...
      <div className="card">
        ...
        <div className="space-y-3">
          {/* Survey Management - Primary Action */}
          {(study.status !== 'draft' && study.status !== 'rejected') && (
            <button
              onClick={() => navigate(`/survey-management/${study.id}`)}
          ...
          {/* Analytics */}
          <button
            onClick={() => navigate(`/analytics/study/${study.id}`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          {(study.status === 'live' || study.status === 'pending-review' || study.status === 'paused' || 
            study.status === 'setup-flow' || study.status === 'redirect-confirmed' || 
            study.status === 'pending-approval' || study.status === 'soft-launch' || 
            study.status === 'pre-launch' || study.status === 'live-link-pending' ||
            study.status === 'redirect-pending' || study.status === 'approved') && (
            <div className="card">
              ...
              <SurveyManagement
                currentStatus={(() => {
                  ...
                  if (study.status === 'setup-flow') return 'redirect-confirmed';
                  if (study.status === 'redirect-confirmed') return 'redirect-confirmed';
                  if (study.status === 'redirect-pending') return 'redirect-pending';
                  if (study.status === 'pending-approval') return 'pre-launch';
                  if (study.status === 'soft-launch') return 'soft-launch';
                  if (study.status === 'pre-launch') return 'pre-launch';
                  if (study.status === 'live-link-pending') return 'pre-launch';
                  if (study.status === 'approved') return 'pre-launch';
                  return 'draft';
                })() as 'draft' | 'live' | 'paused' | 'ended' | 'pending-review' | 'redirect-pending' | 'redirect-confirmed' | 'pre-launch' | 'soft-launch'}
                totalInvites={study.participants.length}
                completed={0}
                pending={study.participants.length}
                failed={0}
                optedOut={0}
                redirectLink={study.configuration?.redirectLink}
                redirectConfirmed={study.configuration?.redirectConfirmed}
                softLaunchConfig={study.configuration?.softLaunchConfig}
